...
class LoadMorePosts
{
  ...
  {
    add_action( 'wp_enqueue_scripts', [$this, 'load_more_scripts'] ); // localize script, add the required variables.
    add_action('wp_ajax_nopriv_more_post_ajax', [$this, 'more_post_ajax']);
    add_action('wp_ajax_more_post_ajax', [$this, 'more_post_ajax']);
  }

  public function load_more_scripts () 
  {
    wp_localize_script( $this->init->scriptName, 'ajax_posts', array(
  ...
  public function more_post_ajax()
  {
    ...
    $posts = []; // to be used to add the posts to it
